package com.example;

import java.util.ArrayList;
import java.util.List;

public class Joker {
    public static void main(String []args){}

    public List<String> getJokeList(){
        List<String> jokeList = new ArrayList<>();
        jokeList.add("A man: “Sorry sir, would you mind if I crossed your field instead of going around it? You see, I have to catch the 4:23 train.”\n" +
                "\n" +
                "The farmer: “Sure, go right ahead. And if my bull sees you, you’ll even catch the 4:11 one.” \n");
        jokeList.add("Scientists have now discovered how women keep their secrets. They do so within groups of 40");
        jokeList.add("My wife’s cooking is so bad we usually pray after our food.\n");
        jokeList.add("Police officer: \"Can you identify yourself, sir?\"\n" +
                " \n" +
                "Driver pulls out his mirror and says: \"Yes, it's me.\"\n"
        );
        jokeList.add("I was in a restaurant once and I suddenly realized I desperately needed to pass gas. The music was really, really loud, so I timed my reliefs to the beat of the music. After just a few songs I started to feel better. I finished my coffee, and noticed that everybody was staring at me... That was when I remembered I was listening to my iPod.\n");
        jokeList.add("\"How much do the potatoes cost?\"\n" +
                "-\n" +
                "\"2.50.\"\n" +
                "-\n" +
                "\"And the bag?\"\n" +
                "-\n" +
                "\"The bag is free.\"\n" +
                "-\n" +
                "\"Ok, give me the bag.\"\n");
        jokeList.add("Fantastic exercise that really helps you to lose weight: Turn your head to the left. Good. Turn your head to the right. Very good. Repeat this exercise whenever you are offered any food.\n"
        );
        jokeList.add("Whatever you do, always give 100%. Unless you are donating blood.");
        jokeList.add("Question: \"Which Day Of The Week Is Most Hated By Fish?\" \n" +
                "Answer: \"Fry Day\"");
        jokeList.add("Santa Wrote Bill Gates About PC & Windows Problems.\n" +
                "\n" +
                "1 My Child Learned MS Word Now He Wants MS Sentence." +
                "2 There Is Only Re-Cycle But No Re-Scooter, I Need It, As I Owe A Vespa Scooter.\n" +
                "\n" +
                "3 I See MS Office But I Need MS Home, As I Use PC At Home.\n" +
                "\n" +
                "4 Finally, I am Confused That Your Name Is Gates But You Are Selling Windows, Why?");

        return jokeList;
    }
}
